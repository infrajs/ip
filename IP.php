...
class IP {
	...
	public static function get ($ip = false, $lang = 'en')
	{
		...
		$conf = Config::get('ip');
		$data = MemCache::exec('Регионы ip адресов', function ($ip, $key, $lang) {
			if (!empty($key)) {
				//$ip = '62.106.100.30';

				$src = 'http://api.ipstack.com/'.$ip.'?access_key='.$key.'&output=json&language='.$lang;
			...
			$data['time'] = time();
			return $data;
		}, array($ip,$conf['key'], $lang), ['akiyatkin\boo\Cache','getDurationTime'], ['last month']);
